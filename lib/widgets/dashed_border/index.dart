...
class UFUDashedBorder extends Border {
  /// You can define sides for each corner.
  const UFUDashedBorder({
  ...
  const UFUDashedBorder.fromBorderSide({
  ...
  const UFUDashedBorder.symmetric({
  ...
  void _drawRadiusPath(
    ...
    //region calculate spacing and draw top border
    partNumber =
        (topWidth + topRightCorner / 2 + topLeftCorner / 2) ~/
        (dashLength + space);
